...
def image_transform_gpu_wrapper(image_size, mean, std, resize_mode, interpolation, aug_cfg):
    def image_transform_gpu(device, input_dtype):
        ...
        train_transform.extend([
            ToTensor(),
            Lambda(lambda x: x.to(device=device, dtype=input_dtype)),
        ])
        train_transform.extend([
            RandomResizedCrop(
                image_size,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            layers: int,
            heads: int,
            vision_tower: str, # ViT-B-16, ViT-L-16, ViT-S-16
            image_processor_name: str, # 'openai/clip-vit-base-patch16' # vision_tower
        ...
        # self.proj = nn.Parameter(scale * torch.randn(pool_dim, output_dim)) changed!
